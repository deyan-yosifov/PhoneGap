function Game15(difficulty) {
    ...
    this.solvedRowColumn = function (cellValue) {
        var zeroBasedValue = parseInt(cellValue - 1);

        var col = zeroBasedValue % size;
        var row = (zeroBasedValue - col) / size;

        return {
            r: row,
            c: col
        };
    };//end this.solvedRowColumn
